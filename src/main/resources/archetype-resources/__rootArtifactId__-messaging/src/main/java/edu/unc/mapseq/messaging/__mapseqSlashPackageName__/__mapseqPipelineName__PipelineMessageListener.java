...
public class ${mapseqPipelineName}PipelineMessageListener implements MessageListener {

    private final Logger logger = LoggerFactory.getLogger(${mapseqPipelineName}PipelineMessageListener.class);

    private PipelineBeanService pipelineBeanService;

    public ${mapseqPipelineName}PipelineMessageListener() {
        super();
    }
            ...
            for (int i = 0; i < entityArray.length(); ++i) {
                ...
                if (entityJSONObject.has("entity_type")) {
                    ...
                    if ("Workflow run".equals(entityType) || WorkflowRun.class.getSimpleName().equals(entityType)) {
                        ...
                        workflowRun = EntityUtil.getWorkflowRun(pipelineBeanService.getMaPSeqDAOBean(), "${mapseqPipelineName}",
